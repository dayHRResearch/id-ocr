...
class myThreshold:
    ...
    def getMinimumThreshold(self, imgSrc):
        ...
        while(self.__IsDimodal(HistGramCC) == False):
            HistGramCC[0] = (HistGramC[0] + HistGramC[0] +
                             HistGramC[1]) / 3.0  # 第一点
            for Y in range(1, 255):
                HistGramCC[Y] = (HistGramC[Y - 1] +
                                 HistGramC[Y] + HistGramC[Y + 1]) / 3  # 中间的点

            HistGramCC[255] = (
                HistGramC[254] + HistGramC[255] + HistGramC[255]) / 3  # 最后一点
        ...
        for Y in range(1, 255):
            if (HistGramCC[Y - 1] < HistGramCC[Y]
                    and HistGramCC[Y + 1] < HistGramCC[Y]):
                Peakfound = True
            if (Peakfound and HistGramCC[Y -
                                         1] >= HistGramCC[Y] and HistGramCC[Y +
                                                                            1] >= HistGramCC[Y]):
                return Y - 1
        return -1
    ...
    def get1DMaxEntropyThreshold(self, imgSrc):
        ...
        for Y in range(MinValue + 1, MaxValue):
            ...
            EntropyBack = 0
            for X in range(MinValue, Y + 1):
                EntropyBack += (- HistGramD[X] / SumIntegral *
                                math.log(HistGramD[X] / SumIntegral))
            ...
            for X in range(Y + 1, MaxValue + 1):
                ...
                EntropyFore += (- HistGramD[X] / (1 - SumIntegral)
                                * math.log(HistGramD[X] / SumI))
